...
const HamburgerIcon = ({ open }) =>
  open ? (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#CFCFCF" strokeWidth="2.5" strokeLinecap="round">
    ...
    <nav className="bg-[#161616] border-b border-[#2a2a2a]">
          ...
          <div className="hidden sm:flex items-center gap-1">
            <button
              ...
              className={`px-5 py-2 rounded text-sm font-medium transition-all duration-200 font-inter ${
                !showSessionNote ? "bg-[#2a2a2a] text-white" : "text-[#707070] hover:text-[#CFCFCF] hover:bg-[#1e1e1e]"
              }`}
            >
              Notes
            </button>
      ...
      {isMobileOpen && (
        <div className="sm:hidden border-t border-[#2a2a2a] px-4 py-3 space-y-1">
          {/* View tabs */}
          <button
            onClick={() => { setShowSessionNote(false); closeMobile(); }}
          ...
          <button
            onClick={() => { setShowSessionNote(true); closeMobile(); }}
            className={`w-full text-left px-4 py-3 rounded text-sm font-medium font-inter transition-colors ${
              showSessionNote ? "bg-[#2a2a2a] text-white" : "text-[#707070] hover:text-[#CFCFCF] hover:bg-[#1e1e1e]"
            }`}
          >
          ...
          {/* Actions */}
          <button
            onClick={() => { handleExportNotes(); closeMobile(); }}
          ...
          <button
            onClick={() => { handleImportNotes(); closeMobile(); }}
            className="w-full text-left px-4 py-3 rounded text-sm text-[#CFCFCF] hover:bg-[#2a2a2a] hover:text-white transition-colors font-inter"
          >
            Import Notes
          </button>
          <button
            onClick={() => { handleResetApp(); closeMobile(); }}
